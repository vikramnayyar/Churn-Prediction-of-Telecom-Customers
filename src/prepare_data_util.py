...
# balancing dataset
def balance_the_dataset(df, label, size):
    # undersampling non-churn class
    to_remove = np.random.choice(df[df[label] == "No"].index, size = size, replace=False) 
    df = df.drop(df.index[to_remove])   
    
    # oversampling churn class
    to_add = np.random.choice(df[df[label] == "Yes"].index, size = size, replace=False) 
